...
public class RecordProgressController {
    ...
    private class ChangeProgressRunnable implements Runnable {
        @Override
        public void run() {
            if (getTotalRecordTime() >= RecordConfig.MAX_DURATION) {
    ...
    public boolean isPassMinPoint() {
        ...
        return recordedTime >= RecordConfig.MIN_DURATION;
    }
    ...
    public void stopRecording() {
        ...
        if (System.currentTimeMillis() - mStartRecordingTime <= 1000) {//当录制的时间小于1秒时，删除录制
            rollback();
        }
    }
    ...
    public int getRecordedTime() {
        return mProgressView.mTotalWidth * RecordConfig.MAX_DURATION / mProgressView.mScreenWidth;
    }
